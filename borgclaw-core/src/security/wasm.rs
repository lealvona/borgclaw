...
impl WasmSandbox {
    ...
    fn execute_wasm_sync(
        ...
        let engine = Engine::new(&config).map_err(|e| {
            super::SecurityError::WasmError(format!("Engine creation failed: {}", e))
        })?;
        ...
        let func = instance
            ...
            .ok_or_else(|| {
                super::SecurityError::WasmError(format!("Function '{}' not found", function))
            })?;
        ...
        let memory = instance
            ...
            .ok_or_else(|| {
                super::SecurityError::WasmError("Memory export not found".to_string())
            })?;
            ...
            if let Some(alloc_func) = alloc_export.into_func() {
                if let Ok(typed_alloc) = alloc_func.typed::<i32, i32>(&store) {
                    typed_alloc.call(&mut store, input_len).map_err(|e| {
                        super::SecurityError::WasmError(format!("Allocation failed: {}", e))
                    })?
                } else {
                    0
                }
        ...
        let typed_func = func.typed::<(i32, i32), i32>(&store).map_err(|e| {
            super::SecurityError::WasmError(format!("Function signature mismatch: {}", e))
        })?;
        ...
        while offset < data.len() {
            ...
            if output.len() > 1024 * 1024 {
                return Err(super::SecurityError::WasmError(
