...
impl WasmSandbox {
    ...
    pub async fn execute(
        ...
        let _permit = self
            ...
            .acquire()
            .await
            .map_err(|e| crate::security::SecurityError::WasmError(format!("Semaphore error: {}", e)))?;

        let modules = self.modules.read().await;

        let wasm_bytes = modules
            .get(module_name)
            .ok_or_else(|| crate::security::SecurityError::WasmError("Module not found".to_string()))?;
        ...
        tokio::task::spawn_blocking(move || Self::execute_wasm_sync(&wasm_bytes, &function, &input))
            .await
            .map_err(|e| crate::security::SecurityError::WasmError(format!("Task join error: {}", e)))?
    }
    ...
    fn execute_wasm_sync(
        ...
        let instance = linker
            ...
            .map_err(|e| crate::security::SecurityError::WasmError(format!("Instantiation failed: {}", e)))?;

        let func = instance
            .get_export(&mut store, function)
            .and_then(|e| e.into_func())
            .ok_or_else(|| {
                crate::security::SecurityError::WasmError(format!("Function '{}' not found", function))
            })?;
            ...
            if let Some(alloc_func) = alloc_export.into_func() {
                if let Ok(typed_alloc) = alloc_func.typed::<i32, i32>(&store) {
                    typed_alloc.call(&mut store, input_len).map_err(|e| {
                        crate::security::SecurityError::WasmError(format!("Allocation failed: {}", e))
                    })?
                } else {
                    0
                }
        ...
        memory
            ...
            .map_err(|e| crate::security::SecurityError::WasmError(format!("Memory write failed: {}", e)))?;
        ...
        let result_ptr = typed_func
            ...
            .map_err(|e| crate::security::SecurityError::WasmError(format!("Execution failed: {}", e)))?;
        ...
        String::from_utf8(output)
            .map_err(|e| crate::security::SecurityError::WasmError(format!("Invalid UTF-8 output: {}", e)))
    }
...
mod tests {
    ...
    async fn wasm_sandbox_list_modules_with_modules() {
        ...
        sandbox.register_module("module1", vec![0x00]).await.unwrap();
        sandbox.register_module("module2", vec![0x01]).await.unwrap();
        sandbox.register_module("module3", vec![0x02]).await.unwrap();
    ...
    async fn wasm_sandbox_register_module_overwrites() {
        ...
        sandbox.register_module("test", vec![0x01, 0x02]).await.unwrap();
    ...
    async fn wasm_sandbox_execute_invalid_wasm_fails() {
        ...
        sandbox.register_module("invalid", invalid_wasm).await.unwrap();
