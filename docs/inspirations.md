...
- Not yet implemented but explicitly tracked here: mem0/OpenMemory-style external memory integration, structured fallback deliverables for failed/stuck jobs, per-channel proxy settings, time-range memory recall filters, PTY/background exec tooling, workspace-layered privacy memory, and broader managed skills lifecycle.
