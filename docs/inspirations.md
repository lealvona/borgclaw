...
- Not yet implemented but explicitly tracked here: CLI-native OAuth completion delivery, per-user OAuth token scoping, persisted browser chat/session history, stronger multi-tenant auth and workspace isolation, gateway auth/rate-limit hardening, and more explicit operator session-state surfaces (`idle`, `running`, `waiting`, `error`).
